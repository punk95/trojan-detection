...
class TrojanDataset(Dataset):
    ...
    def __init__(self, path="../datasets/detection/", directory="train", datasetSize=None, train=True,
                 testTrainSplit=0.8):
        self.path = path
        ...
        if self.directory == "train":
            ...
            for d in dirs:
                xFileNames.append({"info": self.path + "trojan/" + d + "/info.json",
                                   "model": self.path + "trojan/" + d + "/model.pt"})
                y.append(True)
            ...
            for d in dirs:
                xFileNames.append(
                    {"info": self.path + "clean/" + d + "/info.json", "model": self.path + "clean/" + d + "/model.pt"})
                y.append(False)
